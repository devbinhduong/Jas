...
export class MobileMenuToggle {
    ...
    onSubMenuClick(event) {
        ...
        if (!$closestAction.find('.navPage-subMenu').length && !$(event.target).hasClass('navPage-subMenu-title')) {
            return;
        } else {
            if ($closestAction.closest('#custom-menu-mobile').length) {
                event.preventDefault();
            }
        }
        const $parentSiblings = $closestAction.siblings();
        if (!$(event.target).hasClass('navPage-subMenu-action')) {
            ...
            const $closestAction2 = $(event.target);
            const $parent2 = $closestAction2.parent();
            $parent2.toggleClass('is-open');
            const $parentSiblings2 = $parent2.siblings();
            if($parent2.hasClass('is-open')) {
                $parentSiblings2.addClass('is-hidden');
            } else {
