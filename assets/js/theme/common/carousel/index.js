...
export const onSlickCarouselChange = (e, carouselObj, context) => {
    ...
    if ($slider.data("style") === "pRight") {
        $slider.on("setPosition", function () {
            let itemWidth = $(".slick-slide", $slider).outerWidth();
            $(".slick-list", $slider).css("padding-right", itemWidth / 2);
            $(".slick-arrow.slick-next", $slider).css("right", itemWidth / 2 + 20);
        });
    }

    $(".slick-slide:not(.slick-current) > div", $slider).attr("tabindex", "-1");
    $(".slick-slide:not(.slick-current) a", $slider).attr("tabindex", "-1");
    $slider.data("state", null);
};

export default function (context, carousel) {
    ...
    if ($carousel.hasClass("heroCarousel")) {
        ...
        $carousel.on('mouseenter', function () {
            $(this).slick('slickPause').addClass('slick-paused is-paused');
        });
        $carousel.on('mouseleave', function () {
            $(this).slick('slickPlay').removeClass('slick-paused is-paused');
        });
    ...
    const customPaging = isMultipleSlides ? () => '<button data-carousel-dot aria-label="dots" type="button"></button>' : () => { };
